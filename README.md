...
- `--allowed-grafana-urls`: Optional comma-separated Grafana base URL allowlist for URL overrides; entries may start with a `*.` host wildcard (e.g. `https://*.grafana.example.com`). Falls back to `GRAFANA_ALLOWED_URLS`. Requires `--allow-grafana-url-override`; an explicit empty flag disables an inherited list.
- `--denied-grafana-url-wildcard-domains`: Optional comma-separated domains (e.g. `corp.example.com,hosting.example.net`) on or under which `--allowed-grafana-urls` wildcards are refused, in addition to the built-in shared hosting domains. Falls back to `GRAFANA_DENIED_URL_WILDCARD_DOMAINS`. Entries are plain domain names, without scheme, port, or wildcard.
...
The allowlist matches exact base URLs, including scheme, port, and path. An entry may replace the leftmost host label with `*`: `https://*.grafana.example.com` matches `https://eu.grafana.example.com` but not `https://grafana.example.com` or `https://a.eu.grafana.example.com`, because the wildcard covers exactly one DNS label. Scheme, port, and path still match exactly, and the wildcard must be followed by at least two labels with a non-numeric top-level label (`https://*.com` and `https://*.10.0.0` are rejected). Wildcards are also rejected directly on a public suffix (`https://*.co.uk`, `https://*.github.io`) and on or under shared hosting domains where one wildcard would cover other customers' instances: `grafana.net` (Grafana Cloud), `grafana-dev.net`, `grafana-ops.net`, `grafana.azure.com` (Azure Managed Grafana), `grafana.aliyuncs.com` (Alibaba Cloud), `amazonaws.com` (Amazon Managed Grafana and other AWS hostnames), and `aivencloud.com` (Aiven). List those instances exactly. This list cannot cover every hosting provider; add others with `--denied-grafana-url-wildcard-domains`, and only use wildcards on domains you control. Grafana authentication is not an SSRF defense.
