...
- `--denied-grafana-url-wildcard-domains`: Optional comma-separated list of domains on or under which allowlist wildcards are refused, in addition to the built-in shared hosting domains listed below. Falls back to `GRAFANA_DENIED_URL_WILDCARD_DOMAINS` when the flag is not set. Entries are plain domain names such as `hosting.example.net`, without scheme, port, or wildcard.
...
List instances on these domains exactly. This list can't cover every hosting provider. Add other shared domains with `--denied-grafana-url-wildcard-domains`, and only use wildcards on domains you control.
